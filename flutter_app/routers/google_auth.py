...
from flutter_app.schemas.user import UserCreate
import os
...
from flutter_app.schemas.google_oauth import OAuthToken, GoogleUserResponse
...
async def google_login(
    ...
    try:
        ...
        if user:
            ...
            user = google_oauth_service.create(
                background_tasks=background_tasks, db=db, google_response=profile_data
            )
            access_token = user_service.create_access_token(user_id=user.id)
            refresh_token = user_service.create_refresh_token(user_id=user.id)
            response = JSONResponse(
...
@google_auth.get(
    ...
    response_model=GoogleUserResponse,
)
async def google_callback(
    background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)
):
    ...
    if not user:
        ...
        user = google_oauth_service.create(
            background_tasks=background_tasks, google_response=profile_data, db=db
        )
